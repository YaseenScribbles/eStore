...
Public Class VendorList
    ...
    Private Sub LoadData()
        ...
        SQL = "select vendorcode 'Code',vendorname 'Vendor Name',agentname 'Group',v.Address1,v.City,v.State,v.Phone,v.Mobile,'' 'A/C Balance','' '  ',vendorid " _
    ...
    Private Sub AlignReportHeader()
        ...
        TG.Columns(1).DefaultCellStyle.Font = New Font("Arial", 9, FontStyle.Bold)
        TG.Columns(10).Visible = False

        ESSA.AlignHeader(TG, 0, DataGridViewContentAlignment.MiddleLeft, 80)
        ESSA.AlignHeader(TG, 1, DataGridViewContentAlignment.MiddleLeft, 300)
        ESSA.AlignHeader(TG, 2, DataGridViewContentAlignment.MiddleLeft, 120)
        ESSA.AlignHeader(TG, 4, DataGridViewContentAlignment.MiddleLeft, 100)
        ...
        ESSA.AlignHeader(TG, 9, DataGridViewContentAlignment.MiddleLeft, 20)

        ESSA.AlignHeader(TG, 3, DataGridViewContentAlignment.MiddleLeft, wSpaceWidth - 1020)
    ...
    Private Sub txtFWord_KeyDown(sender As Object, e As KeyEventArgs) Handles txtFWord.KeyDown

        If e.KeyCode = Keys.Down Then
            If TG.CurrentRow.Index = TG.Rows.Count - 1 Then Exit Sub
            TG.CurrentCell = TG.Item(0, TG.CurrentRow.Index + 1)
        ElseIf e.KeyCode = Keys.Up Then
            If TG.CurrentRow.Index = 0 Then Exit Sub
            TG.CurrentCell = TG.Item(0, TG.CurrentRow.Index - 1)
        ElseIf e.KeyCode = Keys.Enter Then
            TG_KeyDown(sender, e)
        End If
    ...
    Private Sub TG_KeyDown(sender As Object, e As KeyEventArgs) Handles TG.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            e.SuppressKeyPress = True
            'Vendors.EditVendor(Val(TG.Item(10, TG.CurrentRow.Index).Value))
            'MainWindowX5.ShowForm(Vendors, Vendors.txtVendorCode, True)
        End If
