...
Public Class ManualReturn3
    ...
    '    SQL = "select m.vchno,m.vchdt,v.vendorname,m.quantity,m.netamt from " _
    ...
    '    SQL = "SELECT M.VCHNO,M.VCHDT,V.VENDORNAME,M.REMARKS,P.Plucode,P.PluName,D.QTY Quantity,D.Rate,(D.QTY*D.RATE) Amount," _
    '        & "M.COURIER,M.PODNO,M.PODDT,M.TRANSPORT,M.LRNO,M.LRDT,D.TAXPERC,D.IGST,D.TAXAMT " _
    ...
    '            Rpt.SetParameterValue("ReportHead", "Debit Note")
    ...
    '    SQL = "SELECT COURIER,PODNO,PODDT,TRANSPORT,LRNO,LRDT FROM RVCHMASTER WHERE VCHNO=" & iVchNo
    '    With ESSA.OpenReader(SQL)
    '        If .Read Then
    ...
    '            cmbTransport2.SelectedIndex = cmbCourier2.FindStringExact(.GetString(3).Trim)
    ...
    '    SQL = "UPDATE RVCHMASTER SET COURIER='{0}', PODNO='{1}', PODDT='{2}', TRANSPORT='{3}', LRNO='{4}', LRDT='{5}' WHERE VCHNO={6}"
    '    SQL = String.Format(SQL, cmbCourier2.Text.Trim, txtPODNo2.Text.Trim, Format(dtpPOD2.Value, "yyyy-MM-dd"), cmbTransport2.Text.Trim, txtLRNo2.Text.Trim, Format(dtpLRDt2.Value, "yyyy-MM-dd"), Val(TGGrn.Item(0, TGGrn.CurrentRow.Index).Value))
    '    ESSA.Execute(SQL)
    ...
    '        TaxAmt = (Amount * Val(txtTax.Text)) / 100
    ...
    '        TG.Item(9, NRI).Value = Format(Amount + TaxAmt, "0.00")
    ...
    Private Sub ManualReturn3_Load(sender As Object, e As EventArgs) Handles MyBase.Load
