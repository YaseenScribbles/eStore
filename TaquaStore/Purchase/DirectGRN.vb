...
Public Class DirectGRN
    ...
    Private Sub DirectGRN_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        Select Case e.KeyCode
            ...
            Case Keys.F5
                btnLoadEntry_Click(sender, e)
                'btnReset.PerformClick()
            Case Keys.Escape
                If pnlEntryList.Visible Then pnlEntryList.Hide() : Exit Sub
    ...
    Private Sub txtQty_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtQty.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If txtCode.Text.Trim = "" Then
                ...
                txtCode.Focus()
                Exit Sub
            ElseIf lblCodeType.Text = "New" Then
                If txtBegin.Text = "" Then
                    TTip.Show("Begin value should not be empty", txtBegin, 0, 25, 2000)
                    txtBegin.Focus()
                    Exit Sub
                ElseIf txtStep.Text = "" Then
                    TTip.Show("Step value should not be empty", txtStep, 0, 25, 2000)
                    txtStep.Focus()
                    Exit Sub
                ElseIf txtEnd.Text = "" Then
                    TTip.Show("End value should not be empty", txtEnd, 0, 25, 2000)
                    txtEnd.Focus()
                    Exit Sub
                End If
            ...
            If lblCodeType.Text = "Old" Then

                SQL = "SELECT pluid,plucode,pluname,units,costprice,retailprice " _
                & "from productmaster where vendorId = " & LstVendors.SelectedValue _
                & " and plucode = '" _
                & txtCode.Text & "' order by pluid"

            Else

                SQL = "SELECT pluid,plucode,pluname,units,costprice,retailprice,discount " _
                & "from productmaster where vendorId = " & LstVendors.SelectedValue _
                & " and plucode in (" _
                & CombineCode(txtCode.Text) & ") order by pluid"
    ...
    Private Sub AddItemsToGrid()
        ...
        '// Combine Entry Number

        Dim EntryList As String = ""

        For i As Short = 0 To dgvEntry.Rows.Count - 1
            If Val(dgvEntry.Item(0, i).Value) = 1 Then
                EntryList &= dgvEntry.Item(1, i).Value & ","
            End If
        Next

        EntryList = Mid(EntryList, 1, EntryList.Length - 1)
        ...
        SQL = "Select M.PluId, M.PluCode,M.pluname + '-' + M.Id ,units,costprice,retailprice,discount FROM " _
            & "ProductMaster M, ProductAttributes A WHERE M.PluId = A.PluId " _
            & "And A.EntryNo IN (" & EntryList & ") ORDER BY A.EntryNo, A.Sno"
        ...
        With ESSA.OpenReader(SQL)
            ...
            While .Read
                ...
                TG.Item(2, NRI).Value = .GetString(2)
                TG.Item(3, NRI).Value = .GetString(3)
                TG.Item(4, NRI).Value = Format(1, "0.000")
                TG.Item(5, NRI).Value = Format(.Item(4), "0.0000")
                TG.Item(6, NRI).Value = Format(Val(TG.Item(4, NRI).Value) * Val(TG.Item(5, NRI).Value), "0.00")
                TG.Item(7, NRI).Value = Format(.Item(5), "0.0000")
                TG.Item(8, NRI).Value = .Item(6)
    ...
    Private Sub CalculateTotal()
        ...
        lblBillAmt.Text = Format(TotAmt, "0.00")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        ElseIf IsDate(mebLrDt.Text) = False Then
            ...
            mebLrDt.Focus()
            Exit Sub
        ElseIf Val(lblTItms.Text) <= 0 Then
            TTip.Show("No products to save..!", btnSave, 0, 25, 2000)
            Exit Sub
        ElseIf txtInvNo.Text.Trim = String.Empty Then
            TTip.Show("Please fill invoice number..!", txtInvNo, 0, 25, 2000)
            Exit Sub
        ElseIf ESSA.ISFound(String.Format("select vendorid from grnmaster where vendorid={0} and invno='{1}' and convert(date,invdt) between '{2}' and '{3}' ", LstVendors.SelectedValue, txtInvNo.Text.Trim, SDate, EDate)) Then
            TTip.Show("Invoice number already exists..!", btnSave, 0, 25, 2000)
            Exit Sub
        ElseIf cmbLRPaid.SelectedIndex = 0 AndAlso txtLRAmt.Text.Trim = "" Then
            TTip.Show("Please enter LR Amount..!", txtLRAmt, 0, 25, 2000)
            Exit Sub
        End If
    ...
    Private Sub TaxTotal()
        ...
        lblNett.Text = Format(Val(lblBillAmt.Text) + TAmt, "0.00")
        ...
        Try
            ...
            If Edit = True Then
                SQL = "delete from grnmaster where grnno=" & Val(txtGRNNo.Text) & ";" _
                    ...
                    & "DELETE FROM LR_Payment WHERE GrnNo = " & Val(txtGRNNo.Text) & ";" _
            ...
            For i As Short = 0 To TG.Rows.Count - 1
                ...
                If Val(TG.Item(4, i).Value) > 0 Then
                    ...
                    SQL = "UPDATE ProductMaster SET VendorId = " _
                        & VendorID & ",IsUpdated = 0 WHERE PluId = " _
                        & Val(TG.Item(0, i).Value)

                    Cmd.CommandText = SQL
                    Cmd.ExecuteNonQuery()

                    SQL = $"DELETE FROM PriceMaster WHERE ShopId = {ShopID} AND PluId = {Val(TG.Item(0, i).Value)}"
                    Cmd.CommandText = SQL
                    Cmd.ExecuteNonQuery()

                    SQL = $"INSERT INTO PriceMaster VALUES (
                    {ShopID},
                    {Val(TG.Item(0, i).Value)},
                    {Val(TG.Item(5, i).Value)},
                    {Val(TG.Item(7, i).Value)},
                    {Val(TG.Item(7, i).Value)},
                    {Val(TG.Item(7, i).Value)},1,
                    '{Format(DateTime.Now, "yyyy-MM-dd HH:mm:ss")}',
                    '{Format(DateTime.Now, "yyyy-MM-dd HH:mm:ss")}',
                    {UserID},
                    {UserID},
                    {Val(TG.Item(8, i).Value)})"
            ...
            If Edit Then
                ...
                For k As Short = 0 To TG.Rows.Count - 1

                    SQL = "UPDATE ProductMaster SET CostPrice = " _
                        & Val(TG.Item(5, k).Value) & ", RetailPrice = " _
                        & Val(TG.Item(7, k).Value) & ", WholeSalePrice = " _
                        & Val(TG.Item(7, k).Value) & ", MRP = " _
                        & Val(TG.Item(7, k).Value) & ", IsUpdated = 0 WHERE PluId = " _
                        & Val(TG.Item(0, k).Value)
            ...
            SQL = "INSERT INTO LR_Payment Values ( " _
                ...
                & IIf(cmbLRPaid.SelectedIndex = 0, Val(txtLRAmt.Text), 0) & ")"

            Cmd.CommandText = SQL
            Cmd.ExecuteNonQuery()

            'SQL = $"INSERT INTO PriceMaster
            'SELECT {ShopID},D.PluID,P.CostPrice,P.RetailPrice,P.WholeSalePrice,P.MRP,1,
            ''{Format(DateTime.Now, "yyyy-MM-dd HH:mm:ss")}','{Format(DateTime.Now, "yyyy-MM-dd HH:mm:ss")}',{UserID},{UserID},P.Discount
            'from ProductMaster P
            'INNER JOIN GrnDetails D ON P.PluId = D.Pluid
            'INNER JOIN GrnMaster M ON M.GrnNo = D.GrnNo AND 
            'M.GrnNo = {Val(txtGRNNo.Text)}"
            ...
            If MsgBox("GRN saved successfully, do you want to print lable..!", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                ...
                SQL = "select m.pluid,m.plucode,m.pluname,d.qty,m.retailprice,m.id,substring(a.department,1,len(a.department) - 4) + '-' + a.category,a.style,a.material,a.color from productmaster m,grndetails d,productattributes a where m.pluid=d.pluid and m.pluid = a.pluid " _
    ...
    Private Sub btnFilter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFilter.Click
        ...
        SQL = "select m.grnno,m.grndt,m.invno,v.vendorname,m.totalquantity,m.totalamount,m.billno from " _
        ...
        With ESSA.OpenReader(SQL)
            While .Read
                ...
                TGGrn.Item(3, TGGrn.Rows.Count - 1).Value = .GetString(3)
                TGGrn.Item(4, TGGrn.Rows.Count - 1).Value = .Item(4)
                TGGrn.Item(5, TGGrn.Rows.Count - 1).Value = Format(.Item(5), "0.00")
                TGGrn.Item(8, TGGrn.Rows.Count - 1).Value = .Item(6)
            End While
            .Close()
        End With
    ...
    Private Sub TGGrn_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles TGGrn.CellClick
        ...
        If e.ColumnIndex = 6 Then
            ...
            SQL = "SELECT GRNNO,GRNDT,REFNO,REFDT,INVNO,INVDT,LRNO,LRDT,ORDERNO,VENDORID FROM GRNMASTER WHERE GRNNO=" & TGGrn.Item(0, e.RowIndex).Value & ";" _
                & "SELECT P.PLUID,P.PLUCODE,P.PLUNAME + '-' + P.ID,P.UNITS,D.QTY,D.COSTPRICE,(D.QTY*D.COSTPRICE) AMOUNT,P.RETAILPRICE,P.DISCOUNT FROM " _
                & "PRODUCTMASTER P,GRNDETAILS D WHERE P.PLUID=D.PLUID AND D.GRNNO=" & TGGrn.Item(0, e.RowIndex).Value & " ORDER BY D.SNO ; " _
                & "SELECT PaidStatus,ISNULL(Paid,0) FROM LR_Payment WHERE GrnNo = " & TGGrn.Item(0, e.RowIndex).Value
            ...
            With ESSA.OpenReader(SQL)
                ...
                If .Read Then
                    ...
                    If IsAdmin Then
                        If Val(TGGrn.Item(8, e.RowIndex).Value) > 0 Then
                            btnSave.Enabled = False
                        Else
                            btnSave.Enabled = True
                        End If
                    ...
                    LoadOrders(.Item(9))
                    VendorID = .Item(9)
                    txtVendor.Text = TGGrn.Item(3, e.RowIndex).Value
                ...
                While .Read
                    ...
                    TG.Item(8, TG.Rows.Count - 1).Value = .Item(8)
                ...
                If .Read Then
                    cmbLRPaid.SelectedIndex = IIf(Val(.Item(0)) = 1, 0, 1)
                    txtLRAmt.Text = Format(.Item(1), "0.00")
                Else
        ...
        ElseIf e.ColumnIndex = 9 Then
            ...
            SQL = "delete from grnmaster where grnno=" & Val(TGGrn.Item(0, e.RowIndex).Value) & ";" _
                ...
                & "delete from grntax where grnno=" & Val(TGGrn.Item(0, e.RowIndex).Value) & ";" _
                & "DELETE FROM LR_Payment WHERE GrnNo = " & Val(TGGrn.Item(0, e.RowIndex).Value)
    ...
    Private Sub TGGrn_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles TGGrn.CellContentClick
    ...
    Private Sub btnLoadEntry_Click(sender As Object, e As EventArgs) Handles btnLoadEntry.Click

        If LstVendors.SelectedIndex = -1 Then Exit Sub

        SQL = "SELECT EntryNo, COUNT(*) Quantity FROM ProductAttributes A, ProductMaster P " _
            & "WHERE A.PluId = P.PluId AND P.VendorId = {0} GROUP BY EntryNo " _
            & "ORDER BY EntryNo"

        SQL = String.Format(SQL, LstVendors.SelectedValue)

        dgvEntry.Rows.Clear()
        With ESSA.OpenReader(SQL)
            While .Read
                'dgvEntry.Rows.Add()
                'dgvEntry.Item(0, dgvEntry.Rows.Count - 1).Value = 0
                'dgvEntry.Item(1, dgvEntry.Rows.Count - 1).Value = .Item(0)
                'dgvEntry.Item(2, dgvEntry.Rows.Count - 1).Value = .Item(1)
                dgvEntry.Rows.Add(0, .Item(0), .Item(1))
            End While
            .Close()
        End With
    ...
    Private Sub TG_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles TG.CellEndEdit
        ...
        TG.Item(6, e.RowIndex).Value = Format(Val(TG.Item(4, e.RowIndex).Value) * Val(TG.Item(5, e.RowIndex).Value), "0.00")
        CalculateTotal()

    End Sub

    Private Sub dgvEntry_PreviewKeyDown(sender As Object, e As PreviewKeyDownEventArgs) Handles dgvEntry.PreviewKeyDown
    ...
    Private Sub lblCodeType_Click(sender As Object, e As EventArgs) Handles lblCodeType.Click

        If lblCodeType.Text = "New" Then
            lblCodeType.Text = "Old"
        Else
            lblCodeType.Text = "New"
        End If
