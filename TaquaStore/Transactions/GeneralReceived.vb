...
Public Class GeneralReceived
    ...
    Private Sub txtQty_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtQty.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            TG.Item(4, NRI).Value = TG.Item(4, NRI).Value + Val(txtQty.Text)
            TG.Item(5, NRI).Value = Format(Val(txtRate.Text) * Val(txtQty.Text), "0.00")
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            If Edit = False Then
                GenerateEntryID()
            Else
                SQL = "delete from rcmast where VchNo=" & Val(txtClnNo.Text) & ";" _
                    & "delete from rcdet where VchNo=" & Val(txtClnNo.Text)
                Cmd.CommandText = SQL
                Cmd.ExecuteNonQuery()
            End If

            SQL = "insert into rcmast values (" _
                & Val(txtClnNo.Text) & ",'" _
            ...
            For i As Short = 0 To TG.Rows.Count - 1
                ...
                If Val(TG.Item(4, i).Value) > 0 Then
                    ...
                    SQL = "insert into rcdet values (" _
                        & Val(txtClnNo.Text) & "," _
        ...
        '    PrintBill(txtClnNo.Text, CDate(mebClnDt.Text).Date, cmbCust.Text)
        'End If
    ...
    Private Sub AlterBill()
        ...
        SQL = "select vchno,vchdt,custid,remarks from rcmast where vchno=" & TGAlter.Item(0, TGAlter.CurrentRow.Index).Value & ";" _
            & "select d.pluid,p.plucode,p.pluname,d.rate,d.qty,sum(d.qty*d.rate) from " _
            & "productmaster p,rcdet d where p.pluid=d.pluid and d.vchno=" & TGAlter.Item(0, TGAlter.CurrentRow.Index).Value & " group by d.pluid,p.plucode,p.pluname,d.rate,d.qty,d.sno " _
            & "order by d.sno"
    ...
    Private Sub btnAlter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAlter.Click
        ...
        SQL = "select m.vchno,m.vchdt,c.customername,m.totqty,m.totamt from " _
            & "rcmast m,customers c where m.custid=c.customerid order by m.vchno"
    ...
    Private Sub PrintBill(ByVal iVchNo As Short, ByVal iVchDt As Date, ByVal Cust As String)
        ...
        SQL = "select p.plucode,p.pluname,d.rate,d.qty from " _
            & "productmaster p,rcdet d where p.pluid=d.pluid and d.vchno=" & TGAlter.Item(0, TGAlter.CurrentRow.Index).Value & " order by d.sno"
    ...
    Private Sub TGAlter_CellClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles TGAlter.CellClick
        ...
        If e.ColumnIndex = 5 Then
            ...
            SQL = "SELECT M.VCHNO,M.VCHDT,M.CUSTID,M.REMARKS FROM RCMAST M WHERE M.VCHNO=" & Val(TGAlter.Item(0, e.RowIndex).Value) & ";" _
                & "SELECT D.PLUID,P.PLUCODE,P.PLUNAME,D.RATE,D.QTY,(D.QTY*D.RATE) AMOUNT " _
                & "FROM RCDET D,PRODUCTMASTER P WHERE D.PLUID=P.PLUID AND D.VCHNO=" & Val(TGAlter.Item(0, e.RowIndex).Value) _
                & " ORDER BY D.SNO"

            txtClnNo.Text = Val(TGAlter.Item(0, e.RowIndex).Value)
    ...
    Private Sub btnLoad_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLoad.Click
        ...
        SQL = "select m.vchno,m.vchdt,c.customername,m.totqty,m.totamt from " _
         & "dcmast m,customers c where m.custid=c.customerid and vchno not in (select dcno from rcmast) order by m.vchno"
